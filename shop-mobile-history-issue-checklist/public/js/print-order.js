document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    // mode=internal 內部核簽單（含價格）：走後台 API，需要管理員登入過（跟後台共用同一個瀏覽器分頁群組才會有 sessionStorage）
    // 其他一律當成 vendor 廠商訂購單（不含價格）：走前台 API，資料本身就不含價格欄位
    ...
    const order = showPrice
      ? await Api.get(`/api/admin/orders/${id}`, true)
